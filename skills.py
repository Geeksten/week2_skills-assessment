...
def find_common_items(list1, list2):
    ...
    """
    # the empty list where the common items will be added
    # while going through each item in list1, check if there is a match in list2
    # if there is, add the match to the list
    common_items = []

    for list1_item in list1:
        for list2_item in list2:
            if list1_item == list2_item:
                common_items.append(list1_item)
    return common_items
...
def get_pirate_talk(phrase):
    ...
    for i in range(len(phrase_list)):
        if phrase_list[i] in translator.keys():
            phrase_list[i] = translator[phrase_list[i]]
